...
;(function () {
  log('Loading neon config UI...')

  // ── XHR file helper ───────────────────────────────────────────────────────
  const fs = {
    write: function (filename: string, content: string, cb: (e: Error | null) => void) {
      const x = new jsmaf.XMLHttpRequest()
      x.onreadystatechange = function () {
        if (x.readyState === 4 && cb)
          cb(x.status === 0 || x.status === 200 ? null : new Error('write failed'))
      }
      x.open('POST', 'file://../download0/' + filename, true); x.send(content)
    },
    read: function (filename: string, cb: (e: Error | null, d?: string) => void) {
      const x = new jsmaf.XMLHttpRequest()
      x.onreadystatechange = function () {
        if (x.readyState === 4 && cb)
          cb(x.status === 0 || x.status === 200 ? null : new Error('read failed'), x.responseText)
      }
      x.open('GET', 'file://../download0/' + filename, true); x.send()
    }
  }
  ...
  // ── Theme scanner — safe fn.register (already-registered names are ignored) ─
  function scanThemes (): string[] {
    const themes: string[] = []
    try {
      try { fn.register(0x05,  'cfg_open',    ['bigint', 'bigint', 'bigint'], 'bigint') } catch (_) {}
      try { fn.register(0x06,  'cfg_close',   ['bigint'],                     'bigint') } catch (_) {}
      try { fn.register(0x110, 'cfg_getdents', ['bigint', 'bigint', 'bigint'], 'bigint') } catch (_) {}
      ...
      const fd = fn.cfg_open(pa, new BigInt(0, 0), new BigInt(0, 0))
      if (!fd.eq(new BigInt(0xffffffff, 0xffffffff))) {
        const cnt = fn.cfg_getdents(fd, buf, new BigInt(0, 4096))
  ...
  // ── Layout constants ──────────────────────────────────────────────────────
  const VISIBLE = 8          // rows visible at once (8 × 90 = 720 → fits 270..990)
  const CX      = 960
  const BTN_W   = 840
  const BTN_H   = 74
  ...
  // ── Slot widgets (recycled on scroll) ─────────────────────────────────────
  const slotBgs:    Image[]      = []
  ...
  for (let s = 0; s < VISIBLE; s++) {
    ...
    const bg = new Image({ url: IMG_BTN, x: BTN_L, y: bY, width: BTN_W, height: BTN_H, alpha: 0.08 })
    bg.borderColor = 'rgba(0,255,224,0.18)'; bg.borderWidth = 1
  ...
  arrowUp.text = '▲'; arrowUp.x = CX - 12; arrowUp.y = 244; arrowUp.style = 'scroll'; arrowUp.visible = false
  ...
  for (const [k, h] of [['↑↓', '  Navigate'], ['X', '  Change'], ['O', '  Back & Save']]) {
    const kt = new jsmaf.Text(); kt.text = k;  kt.x = fx;      kt.y = 1052; kt.style = 'footer'
    const ht = new jsmaf.Text(); ht.text = h;  ht.x = fx + 24; ht.y = 1052; ht.style = 'footer'
    jsmaf.root.children.push(kt); jsmaf.root.children.push(ht); fx += 270
  }
  ...
  function renderRows () {
    for (let s = 0; s < VISIBLE; s++) {
      ...
      const vis = idx < TOTAL
      slotBgs[s]!.visible = slotBars[s]!.visible = slotNums[s]!.visible =
        slotLabels[s]!.visible = slotValues[s]!.visible = vis
      if (!vis) continue
      ...
      slotBgs[s]!.alpha       = sel ? 0.20 : 0.08
      slotBgs[s]!.borderColor = sel ? 'rgba(0,255,224,0.72)' : 'rgba(0,255,224,0.18)'
      slotBgs[s]!.borderWidth = sel ? 2 : 1
      slotBars[s]!.alpha      = sel ? 1.0 : 0.28
      slotLabels[s]!.style    = sel ? 'white' : 'muted'
      slotValues[s]!.style    = sel ? 'cyan'  : 'value'
    }
    ...
    fs.read('config.json', function (err: Error | null, data?: string) {
      ...
      try {
        ...
        if (d.config) {
          ...
          C.theme           = (cf.theme && availableThemes.includes(cf.theme)) ? cf.theme : (availableThemes[0] || 'neon')
  ...
  renderRows()
  loadConfig()
  log('Neon config UI loaded — ' + TOTAL + ' options, ' + VISIBLE + ' visible.')
})()
